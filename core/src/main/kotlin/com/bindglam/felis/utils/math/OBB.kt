...
data class OBB(val halfExtents: Vector3f, val center: Vector3f, val rotation: Quaternionf) {
    val vertices: List<Float>
        get() = listOf(
            ...
            Vector3f(halfExtents.x, halfExtents.y, halfExtents.z),
            Vector3f(-halfExtents.x, halfExtents.y, halfExtents.z)
        ).map { it.rotate(rotation, Vector3f()) }.stream().collect({ arrayListOf<Float>() }, { list, vector -> list.add(vector.x); list.add(vector.y); list.add(vector.z) }, { list1, list2 -> list1.addAll(list2) })
